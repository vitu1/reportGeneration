...
s <- read.delim(mapping_file_name, sep='\t', stringsAsFactors = FALSE) %>%
  ...
  mutate(isControl = grepl('positive|negative|vibriolambda|crypto|blank|environmental|mock', SampleID))
...
read.delim("preprocess_summary.tsv") %>%
  mutate(Samples = sub(".json", "", Samples)) %>%
  mutate(num_seq=input/1000000) %>%
  merge(s[c("SampleID", "isControl")], by.y="SampleID", by.x="Samples") %>%
  filter(!isControl) %>%
...
preprocess <- read.delim("preprocess_summary.tsv") %>%
  mutate(Samples = sub(".json", "", Samples)) %>%
...
ggplot(preprocess, aes(x=Sample_num, y=value)) +
  ...
  facet_wrap(quality_by, scales = "free_x", nrow = num_rows) +
...
props <- props[,s$SampleID[!s$isControl]]
...
g_bray = ggplot(pc_df, aes(x=Axis.1, y=Axis.2)) +
...
```{r, fig.width=6, fig.height=5}
g_jac = ggplot(pc_df, aes(x=Axis.1, y=Axis.2)) +
  #geom_point(aes_string(x="Axis.1", y="Axis.2", color=color_by)) +
  theme_bw() +
  scale_shape_discrete(name=shape_by) + 
  scale_colour_discrete(name=color_by) +
...
ko <- read_ko_table("ko_assignments.tsv")
ko <- ko[,-s$SampleID[s$isControl]]

ko <- sweep(ko, 2, colSums(ko), "/")

bc <- vegdist(t(ko), method='bray')
ko_pca <- pcoa(bc)
ko_df <- merge(s, ko_pca$vectors[, 1:3], by.x="SampleID", by.y="row.names")
...
g_uu <- ggplot(ko_df, aes(x=Axis.1, y=Axis.2)) +
  ...
  labs(
    ...
    x = paste0("PCoA axis 1 (", ko_pct[1], "%)"),
    y = paste0("PCoA axis 2 (", ko_pct[2], "%)"))
