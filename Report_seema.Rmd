...
Nextera-XT adapters were trimmed using trimmomatic and Table 1 (see Appendix) shows the number of sequenced reads and number reads retrieved from both strands after adapter trimming.
...
illqc$per_dropped <- (illqc$Dropped *100)/illqc$Input


m <- ggplot(illqc, aes(x=per_dropped))
m + geom_histogram(binwidth = 0.5) +
theme_classic() +
  theme_bw() + 
  xlab("Percentage of reads removed after quality control") +
  ylab("Number of samples") 
```
...
**Figure 4: Percentage of host reads in the samples**
...
m <- ggplot(decontam, aes(x=HumanReadsPercent))
m + geom_histogram(binwidth = 0.5) +
theme_classic() +
  theme_bw() + 
  xlab("Percentage of host reads") +
  ylab("Number of samples") 
```
...
lab_control_idx <- grep('negative|vibriolambda|crypto', colnames(props))
...
lab_control_idx <- grep('negative|vibriolambda|crypto', colnames(ko))
