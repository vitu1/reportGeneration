...
**Figure 1: Average nucleotide quality after adapter trimming and quality control**

Please see attached file, `quality_after.pdf`.

```{r,fig.width=7, fig.height=5, fig.align='center', echo=FALSE}
fp <- file.path(work_dir, 'fastqc_after_trim_quality.tsv')
...
idx_R1 <- grep('_R1\\>', rownames(quality))
idx_R2 <- grep('_R2\\>', rownames(quality))

df <- data.frame(pos=pos,
                mn_R1=colMeans(quality[idx_R1,]),
                std_R1=apply(quality[idx_R1,], 2, sd),
                mn_R2=colMeans(quality[idx_R2,]),
                std_R2=apply(quality[idx_R2,], 2, sd))

melted <- melt(df, pos, c(mn_R1, mn_R2, std_R1, std_R2))
...
colnames(decontam) <- c("Samples","Non-RatReads", "RatReads", "RatReadsPercent" )
...
**Figure 2: Heatmap generated after taxonomic assignments using Metaphlan2**
...
**Figure 3: Heatmap generated after mapping against KEGG database showing presence/absence and abundance of genes in pathways**
